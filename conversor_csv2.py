...
def criar_pdf(dados, nome_pdf):
        ...
        for row in grupo_data:
            ...
            if not current_page_rows:  
                ...
                if primeira_pagina:    
                    ...
                    table_parts.append(Spacer(1,12))
    ...
    # Adicione o filtro para excluir "Exclusivo PCD" do agrupamento
    ...
    if vagas_exclusivas_pcd:
        # Adiciona uma quebra de página se já houver conteúdo na lista
        if table_parts:
            table_parts.append(PageBreak())

        # Criação da tabela para as vagas exclusivas PCD
        table_exclusivo_pcd = Table([["Agência", "Vagas", "Descrição", "Local de Trabalho", "Contrato", "Salário", "Escolaridade", "Experiência",]] +
                                    ...
                                    colWidths=col_widths, rowHeights=row_height, repeatRows=1)
        table_exclusivo_pcd.setStyle(style)

        # Adiciona a tabela das vagas exclusivas PCD
